...
class ServerService {
  ...
  Future<String?> joinServerById(String serverId) async {
    final uid = _currentUid;
    final serverDoc = await _servers.doc(serverId).get();
    if (!serverDoc.exists) return 'Server không tồn tại';

    final existing = await _members(serverId).doc(uid).get();
    if (existing.exists) return 'Bạn đã tham gia server này rồi';
  ...
  Future<String?> joinServerByCode(String code) async {
    final uid = _currentUid;
    ...
    if (snap.docs.isEmpty) return 'Mã mời không tồn tại';

    final serverDoc = snap.docs.first;
